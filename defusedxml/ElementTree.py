...
NOTE ON THE "# nosec" COMMENTS BELOW: this module IS the safe
replacement for xml.etree.ElementTree (that's the whole point of the
defusedxml package) - it has to import the real stdlib names below in
order to wrap them with entity/DTD/external-reference forbidding
behaviour (see DefusedXMLParser and _generate_etree_functions() in
common.py). Static scanners such as bandit cannot tell that these
particular references are the trusted wrapper's own implementation
rather than unsafe direct use, so the known-safe import lines are
annotated with "# nosec" to suppress that false positive.
"""
